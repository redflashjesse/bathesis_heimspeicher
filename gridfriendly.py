...
def cal_grid_friendly(df, soc_max, soc_min, zeit, speichergroessen, eta, c_out, c_in, min_flow_threshold, soc_start=None):
	"""
		...
		for day in unique_days:
			...
			for index, row in df_day.iterrows():
				...
				if p_soll > 0:
					...
					if soc_akt > soc_max:
						if not soc_reached_limit: soc_reached_limit=True
						soc_akt = soc_ist
						p_ist = 0
						soc_delta = 0
				else:
				...
				if not soc_reached_limit:
					last_soc = soc_ist
					pass # save result of this day
				else:
					soc_akt = last_soc
					# Find index of first non-zero value in 'Netzeinspeisung'
					index_first_non_zero = (df_day['Netzeinspeisung'] > 0).idxmax()

					# Find corresponding value of 'current_soc'
					current_soc = df_day.loc[index_first_non_zero, 'current_soc']
