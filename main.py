...
def main():
    ...
    print(len(base_data)) # hier fehlen wohl 2 erinträge
    ...
    for size in speichergroessen:
        ...
        #plot_power(df = batterypower_df, startday=15, endday=16, size=size)
        plot_power_freq_dist(df=batterypower_df, startday=15, endday=16, size=size)
...
def plot_power(df,startday, endday,size):
    ...
    netzbezug = df[f'p_netzbezug_{size}Wh_eigenverbrauch'][startday*1440:endday*1440]
    einspeisung = df[f'p_netzeinspeisung_{size}Wh_eigenverbrauch'][startday * 1440:endday * 1440]
    einspeisung = einspeisung.mul(-1)
    print(einspeisung)
    plt.plot(netzbezug)
    plt.plot(einspeisung)
    plt.show()
...
def plot_power_freq_dist(df, startday, endday, size, binsize = 100):
    ...
    einspeisung = df[f'p_netzeinspeisung_{size}Wh_eigenverbrauch'][startday * 1440:endday * 1440]
    min = einspeisung.max()
    bins_out = math.ceil(min/binsize)
    max = netzbezug.max()
    bins_in = math.ceil(max/binsize)

    print(min, max)
    #print(einspeisung)
    plt.hist(netzbezug, bins=bins_in)
    plt.hist(einspeisung.mul(-1), bins= bins_out )
    plt.show()
